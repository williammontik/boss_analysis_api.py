...
def boss_analyze():
    ...
    try:
        ...
        if lang == "zh":
            ...
            footer = (
                "本报告洞见由 KataChat 的 AI 系统生成，数据来源：\n"
                "1. 我们经家长同意的跨新加坡、马来西亚和台湾的匿名专业档案数据库\n"
                "2. 来自可信 OpenAI 研究和领导力趋势数据集的全球商业基准汇总\n"
                "所有数据均通过我们的 AI 模型处理，以识别具有统计学意义的模式，同时严格遵守 PDPA 合规要求。"
                "每项分析的样本量最低阈值为 1,000+ 数据点。\n"
                "PS：本报告已发送至您的邮箱，24 小时内可查收。如需进一步讨论，欢迎随时联系，我们可安排 15 分钟电话会议。"
            )
        elif lang == "tw":
            lines += [
                "📄 AI-生成報告\n\n工作績效報告\n",
            ...
            footer = (
                "本報告洞見由 KataChat 的 AI 系統生成，數據來源：\n"
                "1. 我們經家長同意的跨新加坡、馬來西亞和台灣的匿名專業檔案數據庫\n"
                "2. 來自可信 OpenAI 研究和領導力趨勢數據集的全球商業基準彙總\n"
                "所有數據均通過我們的 AI 模型處理，以識別具有統計學意義的模式，同時嚴格遵守 PDPA 合規要求。"
                "每項分析的樣本量最低閾值為 1,000+ 數據點。\n"
                "PS：本報告已發送至您的郵箱，24 小時內可查收。如需進一步討論，歡迎隨時聯繫，我們可安排 15 分鐘電話會議。"
            )
        else:
            lines += [
        ...
        # ── 5) Build HTML email with blue footers ───────────────────
        html = f"""
...
<div style="background:#e6f7ff;padding:15px;border-left:4px solid #00529B;margin:20px 0;">
  {footer.splitlines()[0]}<br>
  {'<br>'.join(footer.splitlines()[1:3])}<br>
  <em>{footer.splitlines()[3]}</em>
</div>
