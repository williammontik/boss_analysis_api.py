...
def boss_analyze():
    try:
        ...
        def build_metric(title, labels):
            # Improved values for better visual balance
            return {
                "title": title,
                "labels": labels,
                "values": [random.randint(60, 85) for _ in labels]  # Improved range for better appearance
            }

        # Metrics for Manager’s Performance (consistent with Children)
        metrics = [
        ...
        # Summary Report for Boss Section (Restored)
        summary = f"""
...
"""
        ...
        # Footer Section (Same as your original code)
        footer = """
